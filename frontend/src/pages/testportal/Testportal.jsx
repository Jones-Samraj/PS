...
function Testportal() {
  ...
  const submitTest = async () => {
    ...
    try {
      ...
      await axios.delete(
        `http://localhost:5000/delete-booking/${bookedSlot.id}`
      );
  ...
  const ReviewRequest = () => {
      ...
      try {
        for (const qid of selectedQuestions) {
          await axios.post("http://localhost:5000/submit-review", {
            ...
            isFlagged: flaggedQuestions.includes(qid)
          });
        }
    ...
    return (
      <div className="test-container-review">
        <h2>
          ...
          {isEligible ? (
            <>
                    ...
                    <div key={q.id} className="question-item">
                      <label>
                        ...
                        {flaggedQuestions.includes(q.id) && (
                          <FaFlag color="red" style={{ marginLeft: "5px" }} />
                        )}
                      </label>
                    </div>
                  ))}
              ...
              {selectedQuestions.map((qid) => {
                ...
                return (
                  <div key={qid} className="review-question-container">
                    ...
                    <ul className="review-options">
                      {["option1", "option2", "option3", "option4"].map(
                        (key, i) => (
                          <li key={i}>{question[key]}</li>
                        )
                      )}
                    </ul>
                    <label>
                      {/* Comment for Question {questionIndex + 1}: */}
                      {flaggedQuestions.includes(qid) && (
  ...
  return (
    <div className="testpage-container">
      ...
      <div className="testpage-question-box">
        <div className="question-header">
          ...
          <button
            ...
            title={flaggedQuestions.includes(currentQuestion.id) ? "Unflag this question" : "Flag this question"}
          >
            <FaFlag />
          </button>
        </div>
      ...
      <div className="testpage-question-grid">
        {questions.map((q, i) => (
          <div key={q.id} className="question-number-container">
            <button
            ...
            {flaggedQuestions.includes(q.id) && (
              <FaFlag color="red" size={10} className="question-flag-indicator" />
            )}
          </div>
        ))}
      </div>
