...
SELECT 
  ...
  u.department,
  u.year,
  COUNT(ts.score) AS level_completed,
  (
    SELECT c.name
    FROM test_scores t
    JOIN courses c ON t.course_id = c.id
    WHERE t.user_id = u.id AND t.score >= 1
    ORDER BY c.level DESC
    LIMIT 1
  ) AS highest_level_course
FROM 
  authentication u
LEFT JOIN 
  test_scores ts ON u.id = ts.user_id AND ts.score >= 1
WHERE 
  u.id >= 2
GROUP BY 
  u.id
ORDER BY 
  level_completed DESC;
...
app.get("/api/admin/review-history", (req, res) => {
  const sql = `
    SELECT 
      ...
      qr.created_at
    FROM question_reviews qr
    LEFT JOIN authentication a ON qr.user_id = a.id
